...
export function createOrdersRepo(http: Http): {
  listInRange: (fromIso: string, toIso: string) => Promise<AdminOrderMinimal[]>;
  ...
  async function withItems(
    ...
    const detailed = await Promise.all(
      list.map(async (o) => {
        ...
        if (!o.id) return { ...o, items: [] } as AdminOrderMinimal;
        try {
          const detail = await http.get<{ order?: AdminOrderMinimal }>(
